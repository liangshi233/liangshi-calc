...
let renew = '无'
let information = '测试内容:[4.5.53] 数据随时可能更改，请注意时效性'
...
},
{
  ...
  title: `${qName}治疗量`,
  dmg: ({ params, cons, talent, attr, calc, weapon }, { heal }) => heal( 150 / 100 * ( Math.min( ( params.blPct * ( ( 65 + ( cons >= 2 ? 65 : 0 ) ) * 2 + ( weapon.name === '赤月之形' ? 25 : 0 ) ) + params.blPlus ) , 200 ) / 200 ) * calc(attr.hp) + 150 / 100 * calc(attr.atk) )
}]
...
export const buffs = [
{
  ...
  data: {
    _BondOfLife: ({ params , cons , weapon }) => Math.min( ( params.blPct * ( ( 65 + ( cons >= 2 ? 65 : 0 ) ) * 2 + ( weapon.name === '赤月之形' ? 25 : 0 ) ) + params.blPlus ) , 200 )
  }
},
{
  title: '阿蕾奇诺技能：[红死之宴] 阴翳礼赞状态下，普通攻击将转为红死之宴,使造成的伤害提升[aPlus]；通过这种方式消耗生命之契时，会使元素战技「万相化灰」的冷却缩短[_eCd]秒。',
  data: {
    aPlus: ({ talent , attr , calc , params , cons , weapon }) => calc(attr.atk) * ( ( Math.min( ( params.blPct * ( ( 65 + ( cons >= 2 ? 65 : 0 ) ) * 2 + ( weapon.name === '赤月之形' ? 25 : 0 ) ) + params.blPlus ) , 200 ) / 200 ) * ( talent.a['红死之宴提升值比例'] / 100 ) ),
    _eCd: 0.8
  }
},
{
...
},
{
  ...
  cons: 1,
   data: {
    aPlus: ({ attr , calc , params , cons , weapon }) =>  calc(attr.atk) * ( ( Math.min( ( params.blPct * ( ( 65 + ( cons >= 2 ? 65 : 0 ) ) * 2 + ( weapon.name === '赤月之形' ? 25 : 0 ) ) + params.blPlus ) , 200 ) / 200 ) * ( 100 / 100 ) ),
    _aInterruption: 30
   }
},
{
...
},
{
  ...
  cons: 6,
  data: {
    qPlus: ({ calc , attr , params  , cons , weapon }) => ( calc(attr.atk) * ( ( Math.min( ( params.blPct * ( ( 65 + ( cons >= 2 ? 65 : 0 ) ) * 2 + ( weapon.name === '赤月之形' ? 25 : 0 ) ) + params.blPlus ) , 200 ) / 200 ) * ( 700 / 100 ) ) ),
    aCpct: 10 ,
    aCdmg: 70 ,
    qCpct: 10 ,
    qCdmg: 70
 ...
 'vaporize',
{title: `4.2最后修改：[3.11重置] 显示模式:${NamePath} 排行设置:${rankingOnePath},${rankingTwoPath},${rankingThreePath} 专属排行设置:${gs96ranking} 魔物产球设置:${energy} 更新日志:${renew} 其他信息:${information}`}]

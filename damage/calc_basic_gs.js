...
{
  title: '角色养成度',
  dmg: ({ characterName, level, weapon, talent }) => {
    ...
    if (!typeName) {
      ...
      try {
        let jsonPath = path.join(process.cwd(), `plugins/miao-plugin/resources/meta-gs/character/${typeName}/data.json`) //此路径可替换为你当前使用data.json的路径
    ...
    let score = level * (2 / 9) + (Math.min(29, talent.talentLevel.a + talent.talentLevel.e + talent.talentLevel.q) * (20 / 17) + Math.min(2, Math.max(0, talent.talentLevel.a + talent.talentLevel.e + talent.talentLevel.q - 29)) * (20 / 17 / 2))// + (weapon.level * (1 / 425)) + (artis.level * (4 / 17))
    const totalLength = 20
    let progressBar = ''
    let filledCount = 0
    if (score > 0) {
      filledCount = Math.min(1, Math.floor(score / 5))
      if (score >= 20) {
        filledCount = Math.min(Math.floor(score / 5), totalLength)
      }
    }
...
/*{
  title: `${namejsonPath.name}1命：${namejsonPath.cons["1"]?.desc ?? 'buff读取失败'}`,
  data: {
    let cons1 = namejsonPath.cons["1"]?.desc || []
    if (cons1) {
      if (/提升/.test(cons1)) {
        if (/伤害提升/.test(cons1)) {

        } else if (/提升/.test(cons1)){
